...
const initialState = {
  ...
  currentRoute: null,
  driverRoutes: [],
  // A driver can have more than one vehicle — this holds all of them so
  // routes can be grouped/labeled by vehicle. driverAssignedVehicle is kept
  // for backward compatibility with any existing single-vehicle consumers.
  driverVehicles: [],
  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(getVehicleRoutesForDriver.fulfilled, (state, action) => {
        ...
        state.driverVehicles = action.payload.vehicles || [];
      })
