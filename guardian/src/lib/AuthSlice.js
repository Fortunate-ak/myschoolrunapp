  ...
  async (_, thunkAPI) => {
    try {
      const hasSelectedRoute =
        (await AsyncStorage.getItem("has_selected_route")) === "true";
      return { hasSelectedRoute };
    } catch (error) {
      return thunkAPI.rejectWithValue({ hasSelectedRoute: false });
    }
  },
);
  ...
  extraReducers: (builder) => {
    ...
    builder
      .addCase(loadFromStorage.fulfilled, (state, action) => {
        state.hasSelectedRoute = action.payload.hasSelectedRoute;
      })
      .addCase(loadFromStorage.rejected, (state, action) => {
        state.hasSelectedRoute = action.payload?.hasSelectedRoute ?? false;
      });
  },
});

export const { reset, logout, clearAuthState } = authSlice.actions;
