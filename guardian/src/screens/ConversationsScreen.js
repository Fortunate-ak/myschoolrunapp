...
// ─── Avatar colors ──────────────────────────────────────────────────────────
// Deterministic color per name, so the same person/route always gets the
// same colored avatar instead of every row looking identical.
const AVATAR_COLORS = [
  "#3B82F6",
  "#8B5CF6",
  "#EC4899",
...
export default function ConversationsScreen({ navigation }) {
  ...
  const renderItem = ({ item }) => {
    ...
    // Extract the message text from the last message object
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.bg }]}>
      {/* Header - Updated to match ChatScreen padding */}
      <View
        style={[
          styles.listHeader,
          {
            borderBottomColor: theme.border,
            paddingTop: insets.top + 10, // Same as ChatScreen
          },
        ]}
      >
